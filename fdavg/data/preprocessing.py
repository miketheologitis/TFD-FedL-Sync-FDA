...
def cifar100_worker_dataset(num_workers, i):
    X_train, y_train, _, _ = cifar100_load_data()
...
def cifar100_worker_test_dataset(num_workers, i):
    _, _, X_test, y_test = cifar100_load_data()
...
def cifar100_dataset_fn(global_batch_size, input_context):
    batch_size = input_context.get_per_replica_batch_size(global_batch_size)

    dataset = cifar100_worker_dataset(input_context.num_input_pipelines, input_context.input_pipeline_id)
...
def cifar100_test_dataset_fn(global_batch_size, input_context):
    batch_size = input_context.get_per_replica_batch_size(global_batch_size)

    dataset = cifar100_worker_test_dataset(input_context.num_input_pipelines, input_context.input_pipeline_id)
