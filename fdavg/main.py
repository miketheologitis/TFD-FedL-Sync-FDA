...
STEP_METRICS_PATH = os.path.normpath(os.path.join(SCRIPT_DIR, f'{TMP_DIR}/epoch_metrics'))
...
if __name__ == "__main__":
    ...
    if exper_info['task_index'] == 0:
        ...
        step_metrics_df.to_csv(f"{STEP_METRICS_PATH}/{exper_info['test_id']}.csv", index=False)
        epoch_metrics_df.to_csv(f"{EPOCH_METRICS_PATH}/{exper_info['test_id']}.csv", index=False)
